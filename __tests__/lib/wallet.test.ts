...
describe("isValidTestnetAddress", () => {
  it("should validate native SegWit testnet addresses (tb1...)", () => {
    // Generate a valid testnet address using our wallet function
    const validAddress = generateWalletAddress();
    expect(isValidTestnetAddress(validAddress)).toBe(true);

    // Test known valid testnet bech32 addresses
    const knownValidAddresses = [
      "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx", // P2WPKH
      "tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sl5k7", // P2WSH (longer)
    ];
  ...
  it("should validate legacy testnet addresses", () => {
    // Test legacy testnet address formats
    const legacyTestnetAddresses = [
      "mipcBbFg9gMiCh81Kj8tqqdgoZub1ZJRfn", // P2PKH (starts with 'm')
      "n2eMqTT929pb1RDNuqEnxdaLau1rxy3efi", // P2PKH (starts with 'n')
      "2MzQwSSnBHWHqSAqtTVQ6v47XtaisrJa1Vc", // P2SH (starts with '2')
    ];

    legacyTestnetAddresses.forEach((address) => {
      expect(isValidTestnetAddress(address)).toBe(true);
    });
  });
  ...
  it("should reject mainnet addresses", () => {
    ...
    mainnetAddresses.forEach((address) => {
      expect(isValidTestnetAddress(address)).toBe(false);
    });
  });
  ...
  it("should reject invalid address formats", () => {
    const invalidAddresses = [
      ...
      "tb1", // too short
      "tb1toolongaddressthatexceedsmaximumlengthfortestnetaddresses", // too long
      "bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4X", // invalid characters
      "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4", // wrong checksum
  ...
  it("should handle null, undefined, and non-string inputs", () => {
    expect(isValidTestnetAddress(null as never)).toBe(false);
    expect(isValidTestnetAddress(undefined as never)).toBe(false);
    expect(isValidTestnetAddress(123 as never)).toBe(false);
    expect(isValidTestnetAddress({} as never)).toBe(false);
    expect(isValidTestnetAddress([] as never)).toBe(false);
    expect(isValidTestnetAddress(true as never)).toBe(false);
  });
  ...
  it("should handle addresses with whitespace", () => {
    ...
    expect(isValidTestnetAddress(` ${validAddress} `)).toBe(true);
    expect(isValidTestnetAddress(`\t${validAddress}\n`)).toBe(true);
    expect(isValidTestnetAddress(`  ${validAddress}  `)).toBe(true);
  });

  it("should validate bech32 format strictly", () => {
    // Test various invalid bech32 formats
    const invalidBech32 = [
      "tb1", // too short
      "tb1q", // missing data
      "tb1QUPPERCASE", // uppercase not allowed
      "tb1q_underscore", // invalid character
      "tb1q-dash", // invalid character
      "tb1q.dot", // invalid character
    ];

    invalidBech32.forEach((address) => {
      expect(isValidTestnetAddress(address)).toBe(false);
    });
  });

  it("should correctly identify P2WPKH vs P2WSH addresses", () => {
    // P2WPKH should be 42 characters
    const p2wpkhAddress = generateWalletAddress();
    expect(p2wpkhAddress.length).toBe(42);
    expect(isValidTestnetAddress(p2wpkhAddress)).toBe(true);

    // P2WSH should be 62 characters (if we had one)
    // Note: Our wallet doesn't generate P2WSH, but we should validate them
    const knownP2WSH =
      "tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sl5k7";
    expect(knownP2WSH.length).toBe(62);
    expect(isValidTestnetAddress(knownP2WSH)).toBe(true);
  });
});

describe("validateGeneratedAddress", () => {
  it("should validate addresses generated by our wallet functions", () => {
    // Test with default path
    const address1 = generateWalletAddress();
    expect(validateGeneratedAddress(address1)).toBe(true);

    // Test with custom paths
    const address2 = generateWalletAddress("m/84'/1'/0'/0/1");
    const address3 = generateWalletAddress("m/84'/1'/0'/1/0");
    const address4 = generateWalletAddress("m/84'/1'/1'/0/0");

    expect(validateGeneratedAddress(address2)).toBe(true);
    expect(validateGeneratedAddress(address3)).toBe(true);
    expect(validateGeneratedAddress(address4)).toBe(true);
  });

  it("should validate addresses from HD wallet derivation", () => {
    const mnemonic = generateMnemonic();
    const seed = mnemonicToSeed(mnemonic);
    const hdRoot = generateHDRoot(seed);
    ...
    paths.forEach((path) => {
      const address = deriveTestnetAddress(hdRoot, path);
      expect(validateGeneratedAddress(address)).toBe(true);
    });
  });

  it("should reject non-P2WPKH testnet addresses", () => {
    // Test legacy testnet addresses (should be rejected since we only generate P2WPKH)
    ...
    legacyAddresses.forEach((address) => {
      expect(validateGeneratedAddress(address)).toBe(false);
    });
  });

  it("should reject P2WSH addresses (even though they're valid testnet)", () => {
    // Our wallet only generates P2WPKH, so P2WSH should be rejected
    const p2wshAddress =
      "tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sl5k7";
    expect(validateGeneratedAddress(p2wshAddress)).toBe(false);
  });
  ...
  it("should enforce strict format requirements", () => {
    const validAddress = generateWalletAddress();

    // Valid address should pass
    expect(validateGeneratedAddress(validAddress)).toBe(true);

    // Test strict requirements
    expect(validateGeneratedAddress(validAddress.toUpperCase())).toBe(false); // uppercase
    expect(validateGeneratedAddress(validAddress + "x")).toBe(false); // too long
    expect(validateGeneratedAddress(validAddress.slice(0, -1))).toBe(false); // too short
    expect(validateGeneratedAddress(" " + validAddress)).toBe(false); // leading whitespace
  });

  it("should handle edge cases", () => {
    expect(validateGeneratedAddress("")).toBe(false);
    expect(validateGeneratedAddress("   ")).toBe(false);
    expect(validateGeneratedAddress(null as never)).toBe(false);
    expect(validateGeneratedAddress(undefined as never)).toBe(false);
    expect(validateGeneratedAddress(123 as never)).toBe(false);
  });
  ...
  it("should validate exact length requirements", () => {
    ...
    const tooShort = validAddress.slice(0, 41);
    const tooLong = validAddress + "a";

    expect(validateGeneratedAddress(tooShort)).toBe(false);
    expect(validateGeneratedAddress(tooLong)).toBe(false);
  });
});
...
describe("validateMultipleAddresses", () => {
  it("should validate multiple generated addresses", () => {
    const addresses = [
      ...
      generateWalletAddress("m/84'/1'/1'/0/0"),
    ];

    expect(validateMultipleAddresses(addresses)).toBe(true);
  });

  it("should return false if any address is invalid", () => {
    const addresses = [
      generateWalletAddress(),
      generateWalletAddress("m/84'/1'/0'/0/1"),
      "invalid-address", // This one is invalid
      generateWalletAddress("m/84'/1'/1'/0/0"),
    ];

    expect(validateMultipleAddresses(addresses)).toBe(false);
  });

  it("should handle empty array", () => {
    expect(validateMultipleAddresses([])).toBe(true);
  });

  it("should handle single address", () => {
    const validAddress = generateWalletAddress();
    expect(validateMultipleAddresses([validAddress])).toBe(true);

    const invalidAddress = "invalid";
    expect(validateMultipleAddresses([invalidAddress])).toBe(false);
  });

  it("should reject non-array inputs", () => {
    expect(validateMultipleAddresses(null as never)).toBe(false);
    expect(validateMultipleAddresses(undefined as never)).toBe(false);
    expect(validateMultipleAddresses("not-an-array" as never)).toBe(false);
    expect(validateMultipleAddresses({} as never)).toBe(false);
  });
  ...
  it("should reject arrays with non-string elements", () => {
    ...
    expect(validateMultipleAddresses(mixedArray)).toBe(false);
  });
  ...
  it("should validate large arrays efficiently", () => {
    ...
    const addresses = [];
    for (let i = 0; i < 50; i++) {
      addresses.push(generateWalletAddress(`m/84'/1'/0'/0/${i}`));
    }
...
describe("validateAddressDetailed", () => {
  it("should provide detailed validation for generated addresses", () => {
    const address = generateWalletAddress();
    const result = validateAddressDetailed(address);

    expect(result.isValid).toBe(true);
    expect(result.isTestnet).toBe(true);
    expect(result.isNativeSegWit).toBe(true);
    expect(result.isP2WPKH).toBe(true);
    expect(result.addressType).toBe("P2WPKH");
    expect(result.network).toBe("testnet");
    expect(result.errors).toHaveLength(0);
  });

  it("should detect different address types correctly", () => {
    // Test P2WSH (longer testnet address)
    const p2wshAddress =
      "tb1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sl5k7";
    const p2wshResult = validateAddressDetailed(p2wshAddress);

    expect(p2wshResult.isValid).toBe(true);
    expect(p2wshResult.isTestnet).toBe(true);
    expect(p2wshResult.isNativeSegWit).toBe(true);
    expect(p2wshResult.isP2WPKH).toBe(false);
    expect(p2wshResult.addressType).toBe("P2WSH");
    expect(p2wshResult.network).toBe("testnet");

    // Test legacy P2PKH
    const p2pkhAddress = "mipcBbFg9gMiCh81Kj8tqqdgoZub1ZJRfn";
    const p2pkhResult = validateAddressDetailed(p2pkhAddress);

    expect(p2pkhResult.isValid).toBe(true);
    expect(p2pkhResult.isTestnet).toBe(true);
    expect(p2pkhResult.isNativeSegWit).toBe(false);
    expect(p2pkhResult.isP2WPKH).toBe(false);
    expect(p2pkhResult.addressType).toBe("P2PKH");
    expect(p2pkhResult.network).toBe("testnet");

    // Test legacy P2SH
    const p2shAddress = "2MzQwSSnBHWHqSAqtTVQ6v47XtaisrJa1Vc";
    const p2shResult = validateAddressDetailed(p2shAddress);

    expect(p2shResult.isValid).toBe(true);
    expect(p2shResult.isTestnet).toBe(true);
    expect(p2shResult.isNativeSegWit).toBe(false);
    expect(p2shResult.isP2WPKH).toBe(false);
    expect(p2shResult.addressType).toBe("P2SH");
    expect(p2shResult.network).toBe("testnet");
  });

  it("should detect mainnet addresses and provide errors", () => {
    const mainnetAddress = "bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4";
    const result = validateAddressDetailed(mainnetAddress);

    expect(result.isValid).toBe(false);
    expect(result.isTestnet).toBe(false);
    expect(result.isNativeSegWit).toBe(true);
    expect(result.network).toBe("mainnet");
    expect(result.errors).toContain("Address is for mainnet, not testnet");
  });

  it("should handle invalid inputs with descriptive errors", () => {
    const invalidInputs = [
      { input: "", expectedErrors: ["Address must be a non-empty string"] },
      { input: "   ", expectedErrors: ["Address format not recognized"] },
      {
        input: " tb1valid ",
        expectedErrors: ["Address contains leading/trailing whitespace"],
      },
      {
        input: "invalid-format",
        expectedErrors: ["Address format not recognized"],
      },
    ];

    invalidInputs.forEach(({ input, expectedErrors }) => {
      const result = validateAddressDetailed(input);
      expect(result.isValid).toBe(false);
  ...
  it("should handle null and undefined inputs", () => {
    const nullResult = validateAddressDetailed(null as never);
    expect(nullResult.isValid).toBe(false);
    expect(nullResult.errors).toContain("Address must be a non-empty string");

    const undefinedResult = validateAddressDetailed(undefined as never);
  ...
  it("should provide detailed errors for malformed bech32 addresses", () => {
    const malformedBech32 = "tb1qinvalidchecksum";
    const result = validateAddressDetailed(malformedBech32);
    ...
    expect(result.network).toBe("testnet");
    expect(
      result.errors.some((error) => error.includes("Invalid bech32 format"))
    ).toBe(true);
  });

  it("should validate addresses consistently with other functions", () => {
    // Test that validateAddressDetailed is consistent with other validation functions
    const testAddresses = [
      generateWalletAddress(),
      generateWalletAddress("m/84'/1'/0'/0/1"),
    ...
    testAddresses.forEach((address) => {
      const detailedResult = validateAddressDetailed(address);
      const basicTestnetResult = isValidTestnetAddress(address);
      const generatedResult = validateGeneratedAddress(address);

      // If detailed validation says it's a valid testnet address,
      // basic testnet validation should agree
      if (detailedResult.isValid && detailedResult.isTestnet) {
        expect(basicTestnetResult).toBe(true);
      }

      // If it's not a valid testnet address, basic validation should agree
      if (!detailedResult.isValid || !detailedResult.isTestnet) {
        expect(basicTestnetResult).toBe(false);
      }

      // If it's a valid P2WPKH testnet address, validateGeneratedAddress should agree
      if (detailedResult.isValid && detailedResult.isP2WPKH) {
        expect(generatedResult).toBe(true);
      }
    });
  });

  it("should handle stress testing with many addresses", () => {
    // Test with multiple generated addresses
    for (let i = 0; i < 10; i++) {
      const address = generateWalletAddress(`m/84'/1'/0'/0/${i}`);
      const result = validateAddressDetailed(address);
